...
class OccupancyGripMap:
    ...
    def scan_cb(self, scan_msg):
        ...
        #Calculate current location of the robot in pixels
        origin = self.map_msg.info.origin.position
        x_start = np.rint((odom_map[0]-origin.x-MAP_DIM[0]/2)/CELL_SIZE).astype(int)
        y_start = np.rint((odom_map[1]-origin.y-MAP_DIM[1]/2)/CELL_SIZE).astype(int)
        for i in range(len(scan_msg.ranges)):
            if scan_msg.ranges[i] == np.inf:
                print('infinite range? Ignoring for now')
                continue
    ...
    def ray_trace_update(self, map, log_odds, x_start, y_start, angle, range_mes):
        ...
        # print('x_end', x_end, 'y_end', y_end, 'x_start', x_start, 'y_start', y_start, 'range_mes',range_mes, 'angle', angle)
        rr,cc = ray_trace(int(y_start), int(x_start), int(y_end), int(x_end))
        cells = np.vstack([rr, cc]).T
        # print('cells', cells, 'rr', rr, 'cc', cc)

        for i, cell in enumerate(cells):
            if cell[0] > map.shape[0]-1 or cell[1] > map.shape[1]-1 or cell[0] < 0 or cell[1] < 1:
                break
            if i < cells.shape[0]-NUM_PTS_OBSTACLE:
                log_odds[cell[0],cell[1]]=log_odds[cell[0],cell[1]]-BETA
            else:
                log_odds[cell[0],cell[1]]=log_odds[cell[0],cell[1]]+ALPHA
            p = np.exp(log_odds[cell[0],cell[1]])/(1+np.exp(log_odds[cell[0],cell[1]]))
            map[cell[0],cell[1]] = np.rint(p*100).astype(np.uint8)
